...
class SymmetryConv2d(nn.Module):
    ...
    def __init__(self,
        ...
        self.weight = nn.Parameter(torch.randn(
            (self.out_filters, self.in_filters, kernel_size, kernel_size)))

        self.bias = nn.Parameter(torch.randn(
            (self.out_filters))) if bias else None

    def get_out_shape(self, x):

        H = int(
            np.floor((x.shape[-2] - (self.kernel_size - 1) - 1)/self.stride + 1))
        W = int(
            np.floor((x.shape[-1] - (self.kernel_size - 1) - 1)/self.stride + 1))
    ...
    def forward(self, x, H=None, W=None):
        ...
        out_tensor = torch.zeros(
            (x.shape[0], x.shape[1], self.out_filters, H, W), device=x.device)

        out_tensor[:, 0] = F.conv2d(
            x[:, 0], self.weight, self.bias, self.stride)

        if self.reflection:
            out_tensor[:, 1] = F.conv2d(x[:, 0], torch.flip(
                self.weight, dims=(2,)), self.bias, self.stride)
        ...
        for k in range(1, self.rotation):
            ...
            if self.reflection:
                out_tensor[:, 2*k] = F.conv2d(x[:, 2*k],
                                              w_rot, self.bias, self.stride)
                out_tensor[:, 2*k+1] = F.conv2d(x[:, 2*k+1], torch.flip(
                    w_rot, dims=(2,)), self.bias, self.stride)

            else:
                out_tensor[:, k] = F.conv2d(
                    x[:, k], w_rot, self.bias, self.stride)
